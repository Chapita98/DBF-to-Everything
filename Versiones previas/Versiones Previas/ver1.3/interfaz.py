...
def crear_interfaz(root):
    ...
    def construir_tab_con_tabla(tab, incluir_personalizacion=False, es_schema=False):
        ...
        tk.Label(frm_top, text="Carpeta de entrada:" if es_schema else "Archivos DBF:").grid(row=0, column=0, sticky="e")
        entrada_entrada = tk.Entry(frm_top, state="readonly", width=80)
        entrada_entrada.grid(row=0, column=1, padx=5)
        if es_schema:
            tk.Button(frm_top, text="Seleccionar...", command=lambda: seleccionar_directorio(entrada_entrada, actualizar_tabla)).grid(row=0, column=2)
        else:
            tk.Button(frm_top, text="Seleccionar...", command=lambda: seleccionar_archivos(entrada_entrada)).grid(row=0, column=2)
        ...
        if es_schema:
            ...
            lbl_seleccionados.pack(side="right", padx=5)
        ...
        def limpiar_lista():
            if es_schema:
                ...
                archivos_seleccionados.clear()
                entrada_entrada.config(state="normal")
                entrada_entrada.delete(0, tk.END)
                entrada_entrada.config(state="readonly")
                tabla.delete(*tabla.get_children())
        ...
        tk.Button(frm_botones, text="Extraer Schema" if es_schema else "Convertir", 
                  command=lambda: [actualizar_tabla(), extraer() if es_schema else convertir()]).pack(side="right", padx=5)
