...
class MenuPrincipal(tk.Tk):
    def ventana_tipo_manual(self, ruta_dbf, ruta_excel, callback=None):
        import pandas as pd
        ...
        win.title(f"Intervención manual: {os.path.basename(ruta_dbf)}")
        ancho, alto = 650, 440
        centrar_ventana(win, ancho, alto)
        win.resizable(False, False)

        tk.Label(win, text=f"Archivo: {ruta_dbf}", font=("Arial", 10, "bold"), wraplength=600).pack(pady=8)
        frame = tk.Frame(win)
        ...
        # Campo para modificar el archivo de salida Excel
        salida_frame = tk.Frame(win)
        salida_frame.pack(pady=8)
        tk.Label(salida_frame, text="Archivo de salida Excel:").pack(side=tk.LEFT, padx=5)
        ruta_excel_var = tk.StringVar(value=ruta_excel)
        entry_excel = tk.Entry(salida_frame, textvariable=ruta_excel_var, width=50)
        ...
        def sel_excel():
            ...
            if ruta:
                ruta_excel_var.set(ruta)
        tk.Button(salida_frame, text="Cambiar...", command=sel_excel).pack(side=tk.LEFT, padx=5)
        ...
        def finalizar():
            ...
            try:
                # Guardar el DBF como Excel usando pandas y constraints
                import pandas as pd
                df = pd.DataFrame(iter(DBF(ruta_dbf, load=True)))
                # No hay conversión de tipos real, pero el usuario puede modificar el Excel
                df.to_excel(ruta_excel_var.get(), index=False)
                messagebox.showinfo("Listo", f"Archivo Excel guardado en {ruta_excel_var.get()}")
                win.destroy()
                if callback:
                    callback()
        ...
        tk.Button(win, text="Guardar Excel", command=finalizar, bg="#4A90E2", fg="white", font=("Arial", 12, "bold")).pack(pady=10)
    ...
    def abrir_convertir_raiz_excel(self):
        ...
        def ejecutar():
            ...
            # Frame para centrar el label en la parte inferior
            bottom_frame = tk.Frame(win)
            bottom_frame.pack(side=tk.BOTTOM, fill=tk.X, pady=18)
            progreso_label = tk.Label(bottom_frame, textvariable=progreso_var, font=("Arial", 15, "bold"), fg="#4A90E2")
            progreso_label.pack(anchor="center", pady=0)
            ...
            for idx, (ruta_dbf, ruta_excel) in enumerate(archivos_dbf):
                try:
                    ...
                    if len(df) > MAX_FILAS_EXCEL:
                        ...
                        continue
                    mostrar_porcentaje = len(df) > 10000
                    if mostrar_porcentaje:
                        def progreso_archivo(fila_actual, total_filas):
                            porcentaje = int((fila_actual/total_filas)*100) if total_filas else 100
                            progreso_var.set(f"{idx+1}/{total_archivos} - {os.path.basename(ruta_dbf)} - {porcentaje}%")
                            win.update_idletasks()
                        guardar_dataframe_como_excel(df, ruta_excel, progreso_callback=progreso_archivo)
                        # Al terminar el archivo, dejar el contador en 100%
                        progreso_var.set(f"{idx+1}/{total_archivos} - {os.path.basename(ruta_dbf)} - 100%")
                    else:
                        guardar_dataframe_como_excel(df, ruta_excel)
                        progreso_var.set(f"{idx+1}/{total_archivos} - {os.path.basename(ruta_dbf)}")
                    win.update_idletasks()
                    total += 1
                except Exception as e:
