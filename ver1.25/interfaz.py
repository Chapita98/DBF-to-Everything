...
class Aplicacion(tk.Tk):
    ...
    def crear_widgets(self):
        # Frame superior con título dinámico y botón Salir
        top_frame = tk.Frame(self)
        top_frame.pack(fill=tk.X)

        self.titulo_var = tk.StringVar()
        self.titulo_var.set("Modo Básico")
        self.titulo_label = tk.Label(top_frame, textvariable=self.titulo_var, font=("Arial", 14, "bold"))
        self.titulo_label.pack(side=tk.LEFT, padx=15, pady=5)
        ...
        self.crear_tabla_archivos(self.tab_total)
        ...
        # Actualizar el título cuando se cambia de pestaña
        self.tabs.bind("<<NotebookTabChanged>>", self.actualizar_titulo_pestana)

    def actualizar_titulo_pestana(self, event=None):
        idx = self.tabs.index(self.tabs.select())
        texto = self.tabs.tab(idx, "text")
        self.titulo_var.set(texto)
